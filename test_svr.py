...
def retrieveYUV(img):
    ...
    If input is grayscale, treat it as Y channel and set U,V = 0.
    """
...
def generate_adjacencies(segments, n_segments, img, subsquares):
    adjacency_list = [set() for _ in range(n_segments)]

    for (i, j), value in np.ndenumerate(segments):
        if i < img.shape[0] - 1:
            new_val = segments[i + 1][j]
            if value != new_val and np.linalg.norm(subsquares[value] - subsquares[new_val]) < THRESHOLD:
                adjacency_list[value].add(new_val)
                adjacency_list[new_val].add(value)

        if j < img.shape[1] - 1:
            new_val = segments[i][j + 1]
            if value != new_val and np.linalg.norm(subsquares[value] - subsquares[new_val]) < THRESHOLD:
                adjacency_list[value].add(new_val)
                adjacency_list[new_val].add(value)
...
def apply_mrf(observed_u, observed_v, segments, n_segments, img, subsquares):
    ...
    for _ in range(ICM_ITERATIONS):
        ...
        for k in range(n_segments):
            ...
            for u in np.arange(-U_MAX, U_MAX, 0.01):
                energy = (u - observed_u[k]) ** 2 / (2 * COVAR)
                for adj in adjacency_list[k]:
                    energy += WEIGHT_DIFF * (u - hidden_u[adj]) ** 2
                if energy < u_min:
                    u_min, u_best = energy, u

            for v in np.arange(-V_MAX, V_MAX, 0.01):
                energy = (v - observed_v[k]) ** 2 / (2 * COVAR)
                for adj in adjacency_list[k]:
                    energy += WEIGHT_DIFF * (v - hidden_v[adj]) ** 2
                if energy < v_min:
                    v_min, v_best = energy, v

            new_u[k] = u_best
            new_v[k] = v_best

        if np.linalg.norm(hidden_u - new_u) < ITER_EPSILON and np.linalg.norm(hidden_v - new_v) < ITER_EPSILON:
            break

        hidden_u, hidden_v = new_u, new_v

    return hidden_u, hidden_v


def predict_image(u_svr, v_svr, path, verbose=True, output_file=None):
    ...
    for (i, j), seg in np.ndenumerate(segments):
        centroids[seg] += [i, j]
        count[seg] += 1

    centroids /= count[:, None]

    subsquares = np.zeros((n_segments, SQUARE_SIZE * SQUARE_SIZE))

    for k in range(n_segments):
        top = int(min(max(centroids[k][0], 0), img.shape[0] - SQUARE_SIZE - 1))
        left = int(min(max(centroids[k][1], 0), img.shape[1] - SQUARE_SIZE - 1))
        patch = yuv[top:top + SQUARE_SIZE, left:left + SQUARE_SIZE, 0]
        subsquares[k] = np.abs(np.fft.fft2(patch)).flatten()

    pred_u = clampU(u_svr.predict(subsquares))
    pred_v = clampV(v_svr.predict(subsquares))

    pred_u, pred_v = apply_mrf(pred_u, pred_v, segments, n_segments, img, subsquares)

    for (i, j), seg in np.ndenumerate(segments):
        yuv[i, j, 1] = pred_u[seg]
        yuv[i, j, 2] = pred_v[seg]
...
if __name__ == "__main__":
    ...
    parser.add_argument("file", help="Input grayscale image")
    parser.add_argument("-f", default="svr.model", help="Model filename")
    parser.add_argument("-s", default="output.png", help="Output image")
